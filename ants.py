...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        # If this Place has an exit, then the exit's entrance is set to
        # this Place.
        if exit:
            exit.entrance = self
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                # Checks to see which ant can contain the other
                if self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant):
                    insect.contain_ant(self.ant)
                    self.ant = insect
                else: # Error if neither ant can contain
    ...
    def remove_insect(self, insect):
        ...
        """
        # If the insect has the attribute 'doubled_ants', it is the QueenAnt.
        # The QueenAnt cannot be removed. Else, remove the insect.
        if not hasattr(insect, 'doubled_ants'):
            if insect.is_ant and not hasattr(insect, 'doubled_ants'):
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # A bee will pass if there is not an Ant in the same place or
        # if an Ant does not block the path (such as NinjaAnt).
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        """ This returns True if the ant can contain the other ant.
        Return True if the ant can contain AND if the ant is not already
        containing another ant AND the other ant is not a container."""
...
class HarvesterAnt(Ant):
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 1
        # Add one food to the colony.food
        colony.food += 1
        # END Problem 1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        # To start, look for the nearest bee in the current place of the
        # thrower ant.
        ...
        while place_to_throw is not hive:
            # Place the bee is looking to throw needs to be in its
            # throwing range
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # If the inscect is not watersafe, reduce insects armor to zero
        # to remove insect from water
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4
        # If the amount of damage taken will be greater than the armor,
        # cook the bees!
        if amount >= self.armor:
            self.cook_bees(list(self.place.bees))

        # Reduce armor by amount
        Ant.reduce_armor(self, amount)

    def cook_bees(self, bees):
        """Damage bees in the same place of the FireAnt"""
        for bee in bees:
...
# The WallAnt class
class WallAnt(Ant):
    """The WallAnt does nothing each turn, but the WallAnt has a large armor
    value."""
    ...
    def __init__(self):
        self.armor = 4 # WallAnt will have armor value of 4
# END Problem 7
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        # Damage all the bees that enter the same place
        for bee in bees:
            bee.reduce_armor(self.damage)
        # END Problem 8
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 10
        bee.reduce_armor(bee.armor) # reduce bee's armor to zero by eating bee
        self.digesting = self.time_to_digest # restart digesting timer
        # END Problem 10

    def action(self, colony):
        # BEGIN Problem 10
        if self.digesting: # Check if HungryAnt is digesting
            self.digesting -= 1
        else:# If not digesting, eat a random bee
            # Make copy of bees
            bees = list(self.place.bees)
            if bees:
                # pick a random bee to eat
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 11
        if self.ant: # If there is a contained ant, perform that ant's action
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 12
        # Use BodyguardAnt's action method to perform the action of the
        # contained Ant
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if self is QueenAnt.queen_list[0]:
            self.name = "Queen"
            self.doubled_ants = []
        else:
            self.name = "Impostor"
    ...
    def action(self, colony):
        ...
        if self.name == "Queen":
            ...
            # Check places behind Queen to see what Ants need to double up
            # on armor
            place = self.place
            while place.exit:
                place = place.exit

                if place.ant: # Act only if the place has an Ant
                    ant = place.ant
                    if ant not in self.doubled_ants: # If ant is not in ant list
                        ant.damage = ant.damage * 2  # double its damage
                        self.doubled_ants.append(ant)# add it to the list of
                                                     #ants

                    # Check if this Ant is guarding another Ant.
                    # If it is, double the damage of the guarded Ant.
                    # Then add the guarded Ant to the list, doubled_ants
                    if ant.container == True:
                        guarded_ant = ant.ant
                        if guarded_ant and guarded_ant not in self.doubled_ants:
                            guarded_ant.damage = guarded_ant.damage * 2
                            self.doubled_ants.append(guarded_ant)

            # Queen will then throw something at the nearest bee
            ScubaThrower.action(self, colony)
        else:
            # Impostors don't really do an action
            # except reduce its own armor to 0.
    ...
    def reduce_armor(self, amount):
        ...
        if self.name == "Queen" and self.armor == 0:
            bees_win()# End game. Bees win. Boo.
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # Function used for the bee's action to have the proper effect
    def new_action(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            effect_action(colony)
        else: # When duration for effect, bee will use it's original action
            original_action(colony)

    # Get orginal action of bee
    original_action = bee.action
    # Get effected action
    effect_action = effect(bee.action)
    # Set bee's new action to be the new effected action
